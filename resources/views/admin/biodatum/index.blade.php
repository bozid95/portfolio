...
@section('title', trans('admin.biodatum.actions.index'))

@section('body')

<biodatum-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/biodata') }}'" inline-template>

    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <i class="fa fa-align-justify"></i> {{ trans('admin.biodatum.actions.index') }}
                    <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                        href="{{ url('admin/biodata/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                        {{ trans('admin.biodatum.actions.create') }}</a>
                </div>
                <div class="card-body" v-cloak>
                    <div class="card-block">
                        ...
                        <table class="table table-hover table-listing">
                            <thead>
                                <tr>
                                    ...
                                    <th is='sortable' :column="'id'">{{ trans('admin.biodatum.columns.id') }}</th>
                                    <th is='sortable' :column="'nama'">{{ trans('admin.biodatum.columns.nama') }}</th>
                                    <th is='sortable' :column="'tempat_lahir'">
                                        {{ trans('admin.biodatum.columns.tempat_lahir') }}</th>
                                    <th is='sortable' :column="'tgl_lahir'">
                                        {{ trans('admin.biodatum.columns.tgl_lahir') }}</th>
                                    ...
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                        <span
                                            ...
                                            @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                @click="onBulkItemsClickedAll('/admin/biodata')"
                                        ...
                                        <span class="pull-right pr-2">
                                            <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                @click="bulkDelete('/admin/biodata/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                        </span>
                        ...
                        <div class="no-items-found" v-if="!collection.length > 0">
                            ...
                            <a class="btn btn-primary btn-spinner" href="{{ url('admin/biodata/create') }}"
